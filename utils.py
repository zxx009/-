...
#     """ 从旋转矩阵转换为欧拉角（Roll, Pitch, Yaw），返回度数并确保在 0° 到 360° 之间 """
#     # 计算俯仰角 (Pitch), 翻滚角 (Roll), 航向角 (Yaw)
...
def visualize_axes(ax, q, C, K, scale):
    ...
    # Create arrow points (unit vectors along each axis)
    P = np.matrix([[1, 0, 0],   # X-axis (red)
                   [0, -1, 0],  # Y-axis (green)
                   [0, 0, 1]])  # Z-axis (blue)

    # Rotate arrow points using the quaternion
    R = se3lib.quat2SO3(q)  # Convert quaternion to rotation matrix
    P_r = R * P  # Apply the rotation to the points

    # Translate the points to the object's location
    P_t = np.asarray(P_r) + np.transpose([C])  # Add the translation vector

    # Project the 3D points to the 2D image plane using the camera matrix
    p = P_t / P_t[-1, :]  # Homogeneous coordinates
    c = C / C[-1]  # Normalize the camera location
    p = K * p  # Project using the camera matrix
    c = K * np.matrix(c).transpose()  # Project the camera location

    # Calculate the direction vectors for the arrows
    v = p - c  # Direction vectors
    v = scale * v / np.linalg.norm(v)  # Scale and normalize the vectors

    # Plot the arrows on the image
    ax.arrow(c[0, 0], c[1, 0], v[0, 0], v[1, 0], head_width=10, color='r')  # X-axis (red)
    ax.arrow(c[0, 0], c[1, 0], v[0, 1], v[1, 1], head_width=10, color='g')  # Y-axis (green)
    ax.arrow(c[0, 0], c[1, 0], v[0, 2], v[1, 2], head_width=10, color='b')  # Z-axis (blue)
...
def plot_axes(img, q, C, K, scale):
    ...
    cv2.arrowedLine(img, (c[0, 0], c[1, 0]), (v[0, 0], v[1, 0]), (0, 0, 255), 3)
    cv2.arrowedLine(img, (c[0, 0], c[1, 0]), (v[0, 1], v[1, 1]), (0, 255, 0), 3)
    cv2.arrowedLine(img, (c[0, 0], c[1, 0]), (v[0, 2], v[1, 2]), (255, 0, 0), 3)
