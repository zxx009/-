...
# set_global_policy('mixed_float16')  # 启用混合精度
############################################################
#  Utility Functions
############################################################
        ...
        try:
            ...
            if b == 0:
                batch_image_meta = np.zeros(
                    (batch_size,) + image_meta.shape, dtype=np.float32)
...
#         super(UrsoNet, self).__init__()         #2021.12.30
# class UrsoNet():
#
#     def __init__(self, mode, config, model_dir):
#         """
...
#         # self.set_log_dir()
#         self.keras_model = self.build(mode=mode, config=config)0
class UrsoNet():
    def __init__(self, mode, config, model_dir):
        """
        mode: Either "training" or "inference"
    ...
    #     return model
    def build(self, mode, config):
        """Build UrsoNet architecture with stable add_loss integration."""
        assert mode in ['training', 'inference']
        ...
        h, w = config.IMAGE_SHAPE[:2]
        if h / 2 ** 6 != int(h / 2 ** 6) or w / 2 ** 6 != int(w / 2 ** 6):
            raise Exception("Image size must be divisible by 2^6 (e.g. 256, 320, 384, ... )")

        # === Inputs ===
        input_image = KL.Input(shape=[None, None, config.NR_IMAGE_CHANNELS], name="input_image")
        input_image_meta = KL.Input(shape=[config.IMAGE_META_SIZE], name="input_image_meta")
        tensor_dtype = tf.float16 if config.F16 else tf.float32
        ...
        if mode == "training":
            ...
            if config.REGRESS_KEYPOINTS:
                loc_loss_tensor = KL.Lambda(lambda x: self.mse_loss_graph(*x), name="loc_loss")(
                    [input_gt_loc, loc_pred[0]])
                k2_loss_tensor = KL.Lambda(lambda x: self.mse_loss_graph(*x), name="k2_loss")(
                    [input_gt_k2, loc_pred[1]])
                k3_loss_tensor = KL.Lambda(lambda x: self.mse_loss_graph(*x), name="k3_loss")(
                    [input_gt_k3, loc_pred[2]])
            else:
                if config.REGRESS_LOC:
                    loc_loss_tensor = KL.Lambda(lambda x: self.rel_loss_graph(*x), name="loc_loss")(
                        [input_gt_loc, loc_pred])
                else:
                    loc_loss_tensor = KL.Lambda(lambda x: self.softmax_loss_graph(*x), name="loc_loss")(
                        [input_gt_loc, loc_pred])

                if config.REGRESS_ORI:
                    ori_loss_tensor = KL.Lambda(lambda x: self.one_minus_dot_prod_graph(*x), name="ori_loss")(
                        [input_gt_ori, ori_pred])
                else:
                    ori_loss_tensor = KL.Lambda(lambda x: self.softmax_loss_graph(*x), name="ori_loss")(
                        [input_gt_ori, ori_pred])

            # === Assemble model ===
            if config.REGRESS_KEYPOINTS:
                inputs = [input_image, input_image_meta, input_gt_loc, input_gt_k2, input_gt_k3]
                outputs = [loc_pred[0], loc_pred[1], loc_pred[2]]
            else:
                inputs = [input_image, input_image_meta, input_gt_loc, input_gt_ori]
                outputs = [loc_pred, ori_pred]

            model = KM.Model(inputs, outputs, name='urso_net')

            # === Stable Loss Integration ===
            # Directly attach computed losses to model
            if config.REGRESS_KEYPOINTS:
                model.add_loss(tf.reduce_mean(loc_loss_tensor))
                model.add_loss(tf.reduce_mean(k2_loss_tensor))
                model.add_loss(tf.reduce_mean(k3_loss_tensor))
            else:
                model.add_loss(tf.reduce_mean(loc_loss_tensor))
                model.add_loss(tf.reduce_mean(ori_loss_tensor))

            # === Add metrics for logging ===
            model.add_metric(tf.reduce_mean(loc_loss_tensor), name="loc_loss", aggregation="mean")
            if not config.REGRESS_KEYPOINTS:
                model.add_metric(tf.reduce_mean(ori_loss_tensor), name="ori_loss", aggregation="mean")
            else:
                model.add_metric(tf.reduce_mean(k2_loss_tensor), name="k2_loss", aggregation="mean")
                model.add_metric(tf.reduce_mean(k3_loss_tensor), name="k3_loss", aggregation="mean")

            # === Optional learnable loss weights ===
            if config.LEARNABLE_LOSS_WEIGHTS:
                model.trainable_weights.extend([self.ori_weight, self.loc_weight])
    ...
    #     return loss
    # def softmax_loss_graph(self, y_gt, y_pred):
    #     loss = tf.keras.losses.categorical_crossentropy(y_gt, y_pred)

    #     # Convert ori_weight to same dtype as loss
    #     ori_weight_casted = tf.cast(self.ori_weight, loss.dtype)

    #     loss = loss / tf.exp(ori_weight_casted) + ori_weight_casted
    #     return loss
    def softmax_loss_graph(self, y_gt, y_pred):
        """Loss for classification prediction.
        """
        # Experimental: Adaptive weighting based on Laplace likelihood (Kendall & Cipolla)
        # loss = tf.losses.softmax_cross_entropy(y_gt, y_pred)/tf.exp(self.ori_weight) + self.ori_weight
        #tensorflow2.x版本tf.losses.softmax_cross_entropy已被弃用
        #loss = tf.losses.softmax_cross_entropy(y_gt, y_pred)
        loss = tf.keras.losses.categorical_crossentropy(y_gt, y_pred)
        return loss
    ...
    def load_weights(self, weights_in_path, weights_out_path=None, by_name=False, exclude=None):
        """Modified version of the Keras function with multi-GPU support and
        the ability to exclude some layers from loading.
        exclude: list of layer names to exclude
        """

        if exclude:
            by_name = True  # 排除层时必须按名称加载

        # 直接使用 tf.keras.Model 的内置方法加载权重
        try:
            # 多 GPU 模型需加载到内部模型（如果存在）
            model = self.keras_model.inner_model if hasattr(self.keras_model, "inner_model") else self.keras_model

            # 按名称加载并跳过不匹配的层（兼容排除层逻辑）
            model.load_weights(
                weights_in_path,
                by_name=by_name,
                skip_mismatch=True  # 允许跳过不匹配的层（如 exclude 指定的层）
            )
        except Exception as e:
            raise ValueError(f"Failed to load weights from {weights_in_path}. Error: {str(e)}")
    ...
    def get_urso_weights(self, dataset_name):
        ...
        if dataset_name == "soyuz_hard":
            ...
            # 获取本地文件路径
            local_weights_path = os.path.abspath(os.path.join('models', weights_name))

            # 如果本地存在该权重文件，则直接使用
            if os.path.exists(local_weights_path):
                print(f"Using local weights file: {local_weights_path}")
            ...
            TF_WEIGHTS_PATH = 'https://github.com/pedropro/UrsoNet/releases/download/v1.0/' + weights_name
            print(f"Downloading weights from {TF_WEIGHTS_PATH}")
    ...
    #     else:
    #         optimizer = tf.keras.optimizers.Adam(learning_rate, amsgrad=True, clipnorm=self.config.GRADIENT_CLIP_NORM)
    ...
    #     reg_losses = [
    #         tf.keras.regularizers.l2(self.config.WEIGHT_DECAY)(w) / tf.cast(tf.size(w), tensor_dtype)
    #         for w in self.keras_model.trainable_weights
    #         if 'gamma' not in w.name and 'beta' not in w.name]
    #     # self.keras_model.add_loss(tf.add_n(reg_losses))
    # # ✅ 修复关键点：用 lambda 包装 reg loss
    #     if reg_losses:
    ...
    #     # === 5. Add loss layers as metrics ===
    #     for name in loss_names:
    #         try:
    #             layer = self.keras_model.get_layer(name)
    #         except ValueError:
    #             print(f"⚠️ Layer '{name}' not found in model.")
    #             continue
    ...
    #         loss = (
    #             tf.reduce_mean(output, keepdims=True)
    #             * self.config.LOSS_WEIGHTS.get(name, 1.0)
    #         )

    #         # Add as callable loss
    #         self.keras_model.add_loss(lambda loss=loss: loss)

    #         # Add as metric
    #         self.keras_model.add_metric(loss, name=name, aggregation='mean')

    #     print("✅ Model successfully compiled with optimizer:", type(optimizer).__name__)
    def compile(self, learning_rate, momentum):
        """
        Compile UrsoNet model for TF2.x.
        Works with add_loss() integration (no per-output loss mapping).
        """
        config = self.config
        ...
        tensor_dtype = tf.float16 if config.F16 else tf.float32
        reg_losses = [
            tf.keras.regularizers.l2(config.WEIGHT_DECAY)(w) / tf.cast(tf.size(w), tensor_dtype)
            for w in self.keras_model.trainable_weights
            if 'gamma' not in w.name and 'beta' not in w.name
        ]
        if reg_losses:
        ...
        # === Compile model (loss=None, handled by add_loss) ===
        self.keras_model.compile(optimizer=optimizer)

        # === Logging: show what’s inside ===
        print("✅ Model compiled with add_loss() integrated losses (no output-based loss mapping).")
        print("\n=== MODEL LOSSES ===")
        for l in self.keras_model.losses:
            try:
                print(f"- {l.name if hasattr(l, 'name') else str(l)}")
            except Exception:
                print(f"- {l}")
        print("=====================\n")

        print("=== MODEL METRICS ===")
        for m in self.keras_model.metrics:
            try:
                print(f"- {m.name}")
            except Exception:
                print(f"- {m}")
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        # 数据生成器，只返回 x（不再提供 y）
        def generator_for_add_loss(generator):
            """Wrap data_generator to ensure output format is ([x1, x2, x3, x4],)."""
            for batch in generator:
                # 原始 batch 是 (x1, x2, x3, x4) 或 ([x1,x2,x3,x4], y)
                if isinstance(batch, tuple):
                    if isinstance(batch[0], (list, tuple)) and len(batch) == 2:
                        # 已经是 ([x...], y)
                        x = batch[0]
                    else:
                        # 是 (x1, x2, x3, x4)
        ...
        train_gen_raw = data_generator(train_dataset, self.config, shuffle=True, batch_size=self.config.BATCH_SIZE)
        val_gen_raw = data_generator(val_dataset, self.config, shuffle=True, batch_size=self.config.BATCH_SIZE)

        train_generator = generator_for_add_loss(train_gen_raw)
        val_generator = generator_for_add_loss(val_gen_raw)

        try:
            sample_batch = next(train_generator)
            # unwrap if nested like ([x1,x2,x3,x4],) or (([x...], y))
            ...
            # 如果又多包了一层list，比如 [[img,meta,...]]
            if isinstance(sample_x, (list, tuple)) and len(sample_x) == 1 and isinstance(sample_x[0], (list, tuple)):
                sample_x = sample_x[0]

            print("\n=== DEBUG SAMPLE INPUT ===")
            for i, xi in enumerate(sample_x):
                if hasattr(xi, "shape"):
                    print(f"input[{i}] shape: {xi.shape}, dtype: {xi.dtype}")
                else:
                    print(f"input[{i}] type={type(xi)}")
            print("===========================\n")

        except Exception as e:
            print("ERROR during sample batch debug:", e)
            raise
        ...
        #     history_full
        # ]
        # --- Callbacks: minimal & safe (no TensorBoard/model serialization) ---
        class BatchLogger(tf.keras.callbacks.Callback):
            def __init__(self):
                super().__init__()
                self.ori_loss_acc = []
        ...
        callbacks = [
            # Keep only model checkpoint (weights only) and our batch logger.
        ...
        # DO NOT add TensorBoard here (it calls keras_model_summary -> model.to_json)
        # If you want TensorBoard later, use a custom wrapper that skips set_model() (see option B).
        ...
        print("✅ Model compiled (add_loss structure, no outputs expected).")
        print("Losses:", [l.name if hasattr(l, "name") else l for l in self.keras_model.losses])
        # TODO: 
        # print('Total FLOPs',get_flops(self))
        ...
        if os.name == 'nt':
            # 在Windows环境下完全禁用多进程，避免"pop from empty list"错误
            workers = 0
            use_multiprocessing_flag = False
        else:
            workers = multiprocessing.cpu_count()
            use_multiprocessing_flag = True
        ...
        hist = self.keras_model.fit(
            ...
            max_queue_size=100,
            workers=workers,
            use_multiprocessing=use_multiprocessing_flag,
        )
